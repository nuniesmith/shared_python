...
class FKSSettings(BaseSettings):
    ...
    # Core Configuration (aligned with original settings)
    ...
    FKS_ENVIRONMENT: str = Field(alias="APP_ENV", default="dev", description="Deployment environment")
    ...
    def get_health_check_response(self, status: str = "healthy", **additional_data) -> FKSHealthCheck:
...
def _find_env_file() -> Path | None:
    """Find .env file by walking up the directory tree (max 10 levels)."""
    cur = Path.cwd()
    for _ in range(10):
        candidate = cur / ".env"
        if candidate.exists():
...
def get_settings() -> FKSSettings:
    """Get cached settings instance, searching for .env up the directory tree."""
    env_file = _find_env_file()
    env_file_path = str(env_file) if env_file else None
    return FKSSettings(_env_file=env_file_path)
