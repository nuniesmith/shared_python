"""Market simulation & robustness utilities.

Provides simple slippage model, GBM path generation, Monte Carlo PnL runner,
and out-of-distribution (OOD) score helper.
"""
...
def apply_slippage(price: float, volume: float, liquidity_depth: float, impact_coef: float = 1.0) -> float:
    if liquidity_depth <= 0:
        return price
    slip = impact_coef * price * math.sqrt(max(0.0, volume) / liquidity_depth)
    return price + slip
...
def simulate_gbm(start_price: float, mu: float, sigma: float, steps: int, dt: float = 1 / 252) -> List[float]:
    ...
    for _ in range(steps):
        z = random.gauss(0, 1)
        next_p = prices[-1] * math.exp((mu - 0.5 * sigma ** 2) * dt + sigma * math.sqrt(dt) * z)
...
def ood_score(values: Sequence[float], reference_mean: float, reference_std: float) -> float:
    ...
    acc = 0.0
    for v in values:
        acc += abs((v - reference_mean) / reference_std)
